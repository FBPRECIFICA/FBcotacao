  ...
  try {
    ...
    for (const peca of pecas) {
      ...
      const melhor = opcoes.length > 0 ? {
        ...opcoes[0],
        justificativa: `Menor preço encontrado no MercadoLivre.${items[0]?.shipping?.free_shipping ? ' Frete grátis.' : ''}`
      } : {
        fonte: 'MercadoLivre',
      ...
      resultados.push({
        ...
        observacao: mlData.error === 'forbidden' ? 'Token expirado. Contate o administrador.' : (opcoes.length === 0 ? 'Nenhum resultado encontrado.' : null)
      });
    }
